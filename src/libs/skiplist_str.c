  ...
  for (int i = list->num_levels - 1; i >= 0; i--) {
    ...
    while (level_0_ptr != NULL) {
      ...
      if (current_level_ptr != NULL &&
          lstring_compare(current_level_ptr->data, level_0_ptr->data) == 0) {
        // Node is present at this level: print the data
        printf(NODE_PRESENT_FORMAT, current_level_ptr->data->len,
               current_level_ptr->data->data);
...
int sl_s_find(SkipList *sl, LString *data) {
  ...
  while (level >= 0) {
    if (lstring_compare(data, curr->data) > 0) {
      printf("L%d data(%.*s) > curr->data(%.*s)\n", level, data->len,
             data->data, curr->data->len, curr->data->data);
      prev = curr;
      curr = curr->next[level];
    } else if (lstring_compare(data, curr->data) < 0) {
      printf("L%d data(%.*s) < curr->data(%.*s)\n", level, data->len,
             data->data, curr->data->len, curr->data->data);
      level--;
      curr = prev;
